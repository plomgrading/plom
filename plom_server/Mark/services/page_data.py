    ...
    for page in question_pages.order_by("page_number"):
        ...
        if image:  # fixed pages might not have image if yet to be scanned.
            page_list.append(
                {
                    "id": image.pk,
                    "md5": image.base_image.image_hash,
                    "orientation": image.rotation,
                    "server_path": image.base_image.image_file.path,
                    "included": True,
...
class PageDataService:
        ...
        *,
            ...
            pages_metadata.append(
                {
                    ...
                    "md5": page.image.base_image.image_hash,
                    "included": included,
                    "order": page.page_number,
                    "id": page.image.pk,
                    "orientation": page.image.rotation,
                    "server_path": str(page.image.base_image.image_file.path),
                }
            )
        ...
        for page in (
            ...
            pages_metadata.append(
                {
                    "pagename": pagename,
                    "md5": page.image.image__base_image.image_hash,
    ...
    def get_page_image(self, pk: int, *, img_hash: str | None = None) -> File:
        ...
        """
        if img_hash:
            image = Image.objects.select_related("base_image").get(
                pk=pk, base_image__image_hash=img_hash
            )
        else:
            image = Image.objects.select_related("base_image").get(pk=pk)
        return image.base_image.image_file

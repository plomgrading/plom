...
\begin{tikzpicture}[remember picture, overlay]
  \node [below, text=teal, rotate=90, scale=3, opacity=.25] at (current page.west) {\textsf{demo version~3}};
\end{tikzpicture}
}
...
\newcommand{\mcqmark}[5]{%
	\ifprintanswers
		\ifnum#3=1
			\draw[line width=1.2pt, fill=black] (#1,0.4) rectangle (#2,1.1);
		\else
			\draw[line width=1.2pt] (#1,0.4) rectangle (#2,1.1);
		\fi
	\else
		\draw[line width=1.2pt] (#1,0.4) rectangle (#2,1.1);
	\fi
	\node at (#4,0.75) {\Large #5};
}
\newcommand{\choicefourbox}[4]{%
	\begin{center}
	\begin{tikzpicture}
		\draw[line width=1pt] (0,0) rectangle (12,1.5);
		\mcqmark{0.8}{1.5}{#1}{2.0}{A}
		\mcqmark{3.5}{4.2}{#2}{4.7}{B}
		\mcqmark{6.2}{6.9}{#3}{7.4}{C}
		\mcqmark{8.9}{9.6}{#4}{10.1}{D}
	\end{tikzpicture}
	\end{center}
}
...
\begin{center}
	\emph{This is the id page of ``version 3'' of the midterm; it is usually unused.}
\end{center}
...
\begin{questions}
	...
	\begin{parts}
		...
		\blank%
		\begin{solution}
			Notice that \( x^2-1 = (x-1)(x+1)\) and so
			\[
			...
			\]
		\end{solution}
		\part\(\displaystyle \lim_{x \to +\infty} \dfrac{\sqrt{x^2+2x+5}}{2x-7} \)
		\answerbox{1/2}
		\blank%
		\begin{solution}
			First rewrite
			\[
				\dfrac{\sqrt{x^2+2x+5}}{2x-7}
				= \dfrac{|x|\sqrt{1 + 2/x +5/x^2}}{x(2-7/x)}
			\]
			Now since we take the limit to \(+\infty\) we can simplify this further
			\[
				\dfrac{\sqrt{x^2+2x+5}}{2x-7}
				= \dfrac{\sqrt{1 + 2/x +5/x^2}}{2-7/x}
			\]
			Hence
			\[
				\lim_{x \to +\infty} \dfrac{\sqrt{x^2+2x+5}}{2x-7}
				= \lim_{x \to +\infty} \dfrac{\sqrt{1+2/x+5/x^2}}{2-7/x}
				= 1/2
			\]
		\end{solution}
	...
	\begin{parts}
		\part\(\displaystyle \diff{}{x} \left(x^4 + 3x -\log(x) + 7 - \frac{1}{x} \right) \)
		\blank%
		\begin{solution}
			\begin{align*}
				\diff{}{x} \left(x^4 + 3x -\log(x) + 7 - \frac{1}{x}\right)
				 & = 4x^3 + 3 - \frac{1}{x} + 0 + \frac{1}{x^2}
			\end{align*}
		\end{solution}
		\part\(\displaystyle \diff{}{x} \left(x \cos(x)\right) \)
		\blank%
		\begin{solution}
			Using the product rule
			\begin{align*}
				\diff{}{x} x\cos(x) & = x \diff{}{x} \cos(x) + \cos(x) \diff{}{x}x \\
				                    & = -x\sin(x) + \sin(x)
			\end{align*}
		\end{solution}
		\newpage
	...
	\question[4]
	\begin{parts}
		\part\ Compute the cubic Taylor approximation of \(f(x)= (1+x)^{1/3} \) about \(x=0\).
		\blank\blank\blank%
		\begin{solution}
			We start by computing derivatives
			\begin{align*}
				f(x)    & = (1+x)^{1/3}              & f(0)    & = 1     \\
				f'(x)   & = \frac{1}{3(1+x)^{2/3}}   & f'(0)   & = 1/3   \\
				f''(x)  & = -\frac{2}{9(1+x)^{5/3}}  & f''(0)  & = -2/9  \\
				f'''(x) & = \frac{10}{27(1+x)^{8/3}} & f'''(0) & = 10/27
			\end{align*}
			Then
			\[
				T_3(x) = 1 + \frac{x}{3} - \frac{x^2}{9} + \frac{5 x^3}{81}
			\]
		\end{solution}
		\part\ Hence estimate \( 2^{1/3} \).
		\blank%
		\begin{solution}
			\begin{align*}
				2^{1/3} & \approx T_3(1) = 1 + \frac{1}{3} - \frac{1}{9} + \frac{5}{81} \\
				        & = \frac{81+27-9+5}{81} = \frac{104}{81}
			\end{align*}
		\end{solution}
	\end{parts}
	...
	\question[7] The mass of a sample of Polonium-210, initially 10 grams, decreases at a rate proportional to the mass. After one year, 1 gram remains.
	...
	\begin{solution}
		...
		We are told that
		\[
			Q(0)=10 \qquad Q(1)=1 \qquad \diff{Q}{t}=-kQ
		\]
		...
		\begin{align*}
			1 & =Q(1)=10 e^{-k(1)} & \text{ so} \\
			e^{-k}=1/10
		\end{align*}
		Putting everything together we get
		\[
			Q(t)=10(e^{-k})^t = 10^1 \cdot 10^{-t} = 10^{1-t}
		\]

		Now we need to find \(t\) so that \(Q(t)=Q(0)/2 = 5 \)
		\begin{align*}
			5=Q(t) & =10^{1-t}                                                                                \\
			\log 5 & =(1-t)\log 10                                                                            \\
			1-t    & = \frac{\log 5}{\log 10}                                                                 \\
			t      & = 1 - \frac{\log 5}{\log 10} = \frac{\log 10 - \log 5}{\log 10} = \frac{\log 2}{\log 10}
		\end{align*}
		The half-life of Polonium-210 is $\dfrac{\log 2}{\log 10}$ years.

	\end{solution}
	\blankpage%

	\ifprintanswers{\newpage}\fi
	% for demo we label this question as MCQ
	\renewcommand{\questionlabel}{MCQ}
	\question[3] Answer each multiple-choice item. Each item is worth 1 mark.
	\begin{parts}
		\part What is \(7 + 5\)?

		A) 12 \qquad B) 11 \qquad C) 10 \qquad D) 13

		\choicefourbox{1}{0}{0}{0}

		\part What is \(15 - 8\)?

		A) 6 \qquad B) 7 \qquad C) 8 \qquad D) 9

		\choicefourbox{0}{1}{0}{0}

		\part What is \(4 \times 6\)?

		A) 20 \qquad B) 22 \qquad C) 24 \qquad D) 26

		\choicefourbox{0}{0}{1}{0}
	\end{parts}
	\begin{solution}

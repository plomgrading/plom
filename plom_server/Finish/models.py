...
class ReassemblePaperChore(HueyTaskTracker):
    ...
    pdf_file (FileField): stores the reassembled pdf when it is
        built. Should not be directly exposed to users. Note that the
        name attribute associated with this field should not be
        exposed to users since it is simply the stub of the file which
        django has saved to disc and may contain superfluous
        characters for avoiding collisions.
    display_filename (TextField): stores the filename of the
        reassembled pdf to be returned to users.
    report_pdf_file (FileField): stores the student_report pdf when it
        is built. Should not be directly exposed to users.
    report_display_filename (TextField): stores the filename of the
        report pdf to be returned to users.
    """
...
class BuildSolutionPDFChore(HueyTaskTracker):
    ...
    paper (ForeignKey): a link to the paper for which we are making
        the solution pdf.
    pdf_file (FileField): stores the solution pdf when it is
        built. Should not be directly exposed to users. Note that the
        name attribute associated with this field should not be
        exposed to users since it is simply the stub of the file which
        django has saved to disc and may contain superfluous
        characters for avoiding collisions.
    display_filename (TextField): stores the filename of the solution
        pdf to be returned to users.
    """

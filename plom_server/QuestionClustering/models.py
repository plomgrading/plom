...
class QuestionClusteringChore(HueyTaskTracker):
    """A tracker for clustering students' answers in one selected region.
...
class QVCluster(models.Model):
    """A cluster owned by a question clustering job.

    job: the clustering job that owns this cluster.
    question_idx: the question_index of the question being clustered.
    version: the version of the question being clustered.
    page_num: the page number used in the clustering.
    clusterId: the identifier used to differentiate clusters in a clustering job.
    ...
    class Meta:
        ...
        Current constraints:
            1. The model is unique by (job, clusterId, type) and

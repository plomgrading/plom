...
if ! python3 manage.py plom_database --check-for-database; then
	...
	if [[ "$PLOM_DEMO" -eq 1 ]]; then
		if [ "x$PLOM_DEBUG" = "x0" ]; then
			python3 Launcher/launch_scripts/launch_plom_demo_server --production --port $PORT --stop-after bundles_pushed
		else
			python3 Launcher/launch_scripts/launch_plom_demo_server --development --port $PORT --stop-after bundles_pushed
		fi
	else
		if [ "x$PLOM_DEBUG" = "x0" ]; then

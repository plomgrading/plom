...
const numberOfPages = numberOfPagesSpan.dataset.numberOfPages;
	...
	function updateQuestionVersions() {
	  ...
	  statefulSpec.question.forEach((q) => {
        ...
        function assignSelectedPages(type, qidx = null) {
          ...
          switch (type) {
            case 'dnm': insertDnmPages(selectedPagesArray); break;
            case 'id': insertIdPages(selectedPagesArray); break;
            case 'question':
              if (!qidx)
                alert('must select a question');
              insertQuestionPages(qidx, selectedPagesArray); break;
            case 'newQuestion':
              insertPagesIntoNewQuestion(selectedPagesArray); break;
          }

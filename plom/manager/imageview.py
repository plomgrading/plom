...
class ImageViewWidget(QWidget):
    ...
    def updateImage(self, fnames):
        ...
        # trigger a redraw on update of image
        self.forceRedrawOrSomeBullshit()
    ...
    def updateImage(self, fnames):
        ...
        if fnames is not None:
            ...
            for (n, fn) in enumerate(fnames):
                # make a new pixmapitem if needed
                if n not in self.images:
                    self.images[n] = QGraphicsPixmapItem()
                    self.scene.addItem(self.images[n])
                # set the pixmap
                self.images[n].setPixmap(QPixmap(fn))
                # grab its dimension
                w = self.images[n].pixmap().width()
                h = self.images[n].pixmap().height()
                self.images[n].setTransformationMode(Qt.SmoothTransformation)
                self.images[n].setPos(x, 0)
                self.images[n].setVisible(True)
                sf = float(ScenePixelHeight) / float(h)

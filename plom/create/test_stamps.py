...
from plom.create.mergeAndCodePages import pdf_page_add_labels
...
def test_staple_marker_diagname_too_long(tmpdir):
    ...
    pdf_page_add_labels(d[0], "Mg " * 7, "bar", [], even=True)
    d.save("debug_staple_position.pdf")  # uncomment for debugging
    with raises(AssertionError):
        pdf_page_add_labels(d[0], "Mg " * 8, "bar", [], even=True)
    with raises(AssertionError):
        pdf_page_add_labels(d[0], "Mg " * 8, "bar", [], even=False)
    # but no error if we're not drawing staple corners
    pdf_page_add_labels(d[0], "Mg " * 8, "bar", [], even=None)
    d.close()
...
def test_stamp_too_long(tmpdir):
    ...
    assert buildDemoSourceFiles(basedir=tmpdir)
    d = fitz.open(tmpdir / "sourceVersions/version1.pdf")
    pdf_page_add_labels(d[0], "foo", "1234 Q33 p. 38", [], even=True)
    with raises(AssertionError):
        pdf_page_add_labels(d[0], "foo", "MMMM " * 5, [], even=True)
    d.close()

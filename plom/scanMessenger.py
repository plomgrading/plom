...
class ScanMessenger(BaseMessenger):
    ...
    def uploadTestPage(
        self, code, test, page, version, fname, md5sum, bundle, bundle_order
    ):
        self.SRmutex.acquire()
        try:
            param = {
            ...
            dat = MultipartEncoder(
                fields={
                    ...
                    "originalImage": (fname.name, open(fname, "rb"), mime_type),
                }
            )
            response = self.session.put(
    ...
    def uploadHWPage(self, sid, question, order, fname, md5sum, bundle, bundle_order):
        self.SRmutex.acquire()
        try:
            param = {
                "user": self.user,
    ...
    def uploadLPage(self, sid, order, fname, md5sum, bundle, bundle_order):
        self.SRmutex.acquire()
        try:
            param = {
                "user": self.user,
    ...
    def uploadUnknownPage(self, fname, order, md5sum, bundle, bundle_order):
        self.SRmutex.acquire()
        try:
            param = {
                "user": self.user,

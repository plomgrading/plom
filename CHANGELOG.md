...
### Changed
* The format of extra sheets has changed.

### Fixed
* Fix some regressions when creating rubrics via the web UI or from CSV files.
...
* the list of staged bundles now shows whether there are any collisions with pushed bundles: previously you had to "drill down" to find this information.

### Changed
* the web UI for making rubrics did not respect the rubric permissions settings of the server.
* autogenerated rubrics ("system rubrics") can be unpublished by manager users to hide them from clients.
* system rubrics can be modified by manager users.
* unpublished rubrics are shown greyed out in the web UI.

### Fixed
* fixed a crash when pushing new scans of an existing prenamed paper.
...
[Unreleased]: https://gitlab.com/plom/plom/-/compare/v0.21.2...main

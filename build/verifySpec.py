...
def check_keys(spec):
    ...
    if "1" in spec["mark"]:
        print('\tcontains at least 1 marking-group (ie "mark.1") - check')
    else:
        print(
...
def check_pages(spec):
    ...
    for g in range(spec["numberOfGroups"]):
        for p in spec["mark"][str(g + 1)]["pages"]:
            pageUse[p] += 1
...
for g in range(spec["numberOfGroups"]):
    check_group(spec["mark"][str(g + 1)], lastPage)
...
if "privateCode" in spec:
    ...
    spec["privateSeed"] = str(random.randrange(0, 10 ** 16)).zfill(16)
...
if "publicCode" in spec:
    ...
    spec["publicCode"] = str(random.randrange(0, 10 ** 6)).zfill(6)

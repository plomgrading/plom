...
def make_toml(assignment, marks: List[int], *, dur: Union[str, Path] = ".") -> None:
    ...
    # The next variable is expected by legacy PLOM,
    # where its value must be at least the number of questions.
    # The value is ignored in homework mode, so we just write
    # the smallest compatible number and drop a comment in the spec.
    # The expected and observed page counts in real life are handled
    # elsewhere.
    ...
    toml = dedent(
        ...
        # Note that numberOfPages is vestigial, ignored, and possibly unreliable
        name = "{name}"
        longName = "{longName}"

...
class CoreConnectionServiceTests(TestCase):
    ...
    def test_get_latest_init_db_task(self):
        ...
        task1 = baker.make(CoreDBinitialiseTask, created=datetime.now(), status='queued')
        task2 = baker.make(CoreDBinitialiseTask, created=datetime.now() + timedelta(minutes=1), status='running')

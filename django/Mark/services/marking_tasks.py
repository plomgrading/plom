...
class MarkingTaskService:
    ...
    def get_user_mark_results(self, user, question, version):
        """
        Get all the marking actions that were generated by this user.
        ...
        Returns:
            list [MarkAction]: the relevant actions
        """

        actions = MarkAction.objects.filter(user=user)
        if actions:
            if question:
                actions = filter(lambda a: a.task.question_number == question, actions)
            if version:

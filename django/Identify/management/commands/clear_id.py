...
class Command(BaseCommand):
    """Command tool for deleting certain ID paper number or all ID papers
    ...
    help = "Clear the ID of a specific paper or clear all IDs. \
            To delete all IDs: python manage.py clear_id all\
            To delete a specific ID: python manage.py clear_id (paper number)"

    def specific_id(self, paper_num):
        try:
            IDService().set_id__task_todo_and_clear_specific_id_cmd(paper_num)    
            self.stdout.write(f"Deleted ID for paper number #{paper_num}")
        except ObjectDoesNotExist as err:
            raise CommandError(err)

    def clear_all_ids(self):
        self.stdout.write("Deleted all ids.")
